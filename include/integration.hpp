...
Eigen::VectorXd EvaluateExtraTerm(
    const parametricbem2d::ParametrizedMesh &mesh,
  ...
  for (unsigned i = numpanels_i; i < numpanels; ++i) {
    ...
    for (unsigned k = 0; k < Q; ++k) {
      auto integrand = [&](double s) {
        // Jacobian is included in F
        return F(pi, space_d, nu, k, s) * space_n.evaluateShapeFunction(k, s);
      };
      double integral = 0;
      for (unsigned l = 0; l < N; ++l) {
        integral += GaussQR.w(l) * integrand(GaussQR.x(l));
      }
