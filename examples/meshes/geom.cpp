...
int main(int argc, char *argv[]) {
  ...
  std::string filename = "kite_sq" + std::string(argv[1]) + ".geo";
  ...
  unsigned h_inv = 1 << (level - 2);
  unsigned M = h_inv / h_sq * s  - 1;
  // Working size
  unsigned N = M+2;
  ...
  unsigned N_kite = ceil(lkite * h_inv / h_kite);
  ...
  out << "Plane Surface(2) = {2};" << std::endl;
  out << "Physical Surface(\"Outer domain\",3) = {1};" << std::endl;
  out << "Physical Surface(\"Inner domain\",4) = {2};" << std::endl;
  return 0;
}
